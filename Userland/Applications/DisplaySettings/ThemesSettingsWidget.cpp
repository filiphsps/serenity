    ...
    : m_background_settings_changed { background_settings_changed }
{
    ...
    GUI::Application::the()->on_theme_change = [&]() {
        auto theme_override = GUI::ConnectionToWindowServer::the().system_theme_override();
...
{
    if (m_selected_theme && m_selected_theme->name != GUI::ConnectionToWindowServer::the().system_theme())
