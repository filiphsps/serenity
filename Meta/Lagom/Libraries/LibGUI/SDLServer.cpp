    ...
    m_process_loop = Core::Timer::create_repeating(16, [this] {
        ...
        while (SDL_PollEvent(&event)) {
            ...
            /*case SDL_MOUSEMOTION: {
                ...
                ConnectionToWindowServer::the().mouse_move(window->window_id(), Gfx::IntPoint(event.motion.x, event.motion.y), 0, buttons, 0, 0, 0, false, Vector<String>());
                break;
            }
            case SDL_MOUSEBUTTONDOWN: {
                auto window = Window::from_window_id(get_window_from_sdl_id(event.window.windowID));
                if (buttons > 0)
                    ConnectionToWindowServer::the().mouse_down(window->window_id(), Gfx::IntPoint(event.motion.x, event.motion.y), buttons, buttons, 0, 0, 0);
                break;
            }
            case SDL_MOUSEBUTTONUP: {
                ...
                if (buttons > 0)
                    ConnectionToWindowServer::the().mouse_up(window->window_id(), Gfx::IntPoint(event.motion.x, event.motion.y), buttons, buttons, 0, 0, 0);
                break;
            }*/
            case SDL_WINDOWEVENT: {
                ...
                /*case SDL_WINDOWEVENT_RESIZED: {
                    ConnectionToWindowServer::the().window_resized(window->window_id(), Gfx::IntRect(0, 0, event.window.data1, event.window.data2));
                    break;
                }*/
